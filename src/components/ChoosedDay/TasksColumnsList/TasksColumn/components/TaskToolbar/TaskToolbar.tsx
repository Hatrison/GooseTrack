...
import { Props } from './TaskToolbar.types';
import { TTask } from 'redux/tasks/tasks.types';

export const TaskToolbar = ({ handleDeleteTask, item, title }: Props) => {
  const [isChangeDirOpened, setIsChangeDirOpened] = useState(false);
  const dispatch = useAppDispatch();

  const ChangeDirTitle = (title: string) => {
  ...
  const modifyCategory = (task: TTask) => {
    const { category } = task;
